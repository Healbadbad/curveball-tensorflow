...
def conv_model(features, labels, mode):
  ...
  print(labels_onehot)
  loss = tf.losses.softmax_cross_entropy(labels_onehot, logits=logits)

  # Create training op.
  if mode == tf.estimator.ModeKeys.TRAIN:
   # optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.01)
    curveball_optimizer = curveball.CurveballOptimizer(learning_rate=1, name='Curveball', input_to_loss=logits)
    train_op = curveball_optimizer.minimize(loss, global_step=tf.train.get_global_step())
